...
def create_loss_plots(results):
    ...
    blues = LinearSegmentedColormap.from_list('', ['lightsteelblue', 'navy'])(np.linspace(0, 1, num_models))
    reds = LinearSegmentedColormap.from_list('', ['mistyrose', 'darkred'])(np.linspace(0, 1, num_models))
    ...
    for idx, (model_key, num_params) in enumerate(model_params):
        ...
        label = f"e{config['embedding_dim']}_h{config['hidden_dim']}_d{config['depth']} ({num_params:,} params)"
        
        # Plot with color gradients
        ax1.plot(epochs, train_losses, marker='o', markersize=4, label=label, 
                color=blues[idx], linewidth=2)
        ax2.plot(epochs, val_losses, marker='o', markersize=4, label=label, 
                color=reds[idx], linewidth=2)
