...
def predict_with_model(batch_atoms, model_path):
    ...
    # Predict properties
    predicted_energy, predicted_forces, predicted_stress = model.predict_batch(
        batch_atoms
    )
...
if __name__ == "__main__":
    # Setup dataset
    train_dataset_name = "rattled-300-subsampled"
    validation_dataset_name = "rattled-1000"
    for dataset_name in [train_dataset_name, validation_dataset_name]:
        ...
        for batch_idx in tqdm(range(num_batches), desc="Batch Processing"):
            ...
            batch_predictions = predict_with_model(
                batch_atoms,
                model_path=Path(
                    f"checkpoints/{train_dataset_name}_naive_atom_model.pkl"
                ),
            )
            pred_energies, pred_forces, pred_stresses = (
                batch_predictions[:][0].clone().detach(),
                batch_predictions[:][1].clone().detach(),
                batch_predictions[:][2].clone().detach(),
            )
            true_energies, true_forces, true_stresses = zip(*batch_true_properties)
            true_energies, true_stresses = torch.tensor(
                np.array(true_energies)
            ), torch.tensor(np.array(true_stresses))
        ...
        print(
            f"Dataset Name: {dataset_name}, Average Loss Per Batch: {loss / num_batches}"
        )
