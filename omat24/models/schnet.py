...
class InteractionBlock(nn.Module):
    ...
    def __init__(self, hidden_channels, num_filters, num_gaussians, cutoff, device="cpu"):
        """Args:
...
class CFConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, num_filters, nn, cutoff, device="cpu"):
        """Args:
...
class SchNet(nn.Module):
    ...
    def __init__(
        ...
        self.distance_expansion = GaussianSmearing(start=0.0, stop=cutoff, num_gaussians=num_gaussians, device=device)

        # Build interaction blocks
        self.interactions = nn.ModuleList([
            InteractionBlock(hidden_channels, num_filters, num_gaussians, cutoff, device=device)
    ...
    def forward(self, data):
        ...
        if isinstance(data, dict):
            ...
            z = data.atomic_numbers
            pos = data.pos
            batch = data.batch if hasattr(data, "batch") else torch.zeros(z.shape[0], dtype=torch.long, device=self.device)
        ...
        if hasattr(data, "edge_index"):
            ...
            if self.device != "cpu":
                edge_index = radius_graph(pos.to("cpu"), r=self.cutoff, batch=batch, max_num_neighbors=self.max_num_neighbors).to(self.device)
            else:
