...
class EquiformerS2EF(nn.Module):
    ...
    def __init__(self, backbone_config):
        ...
        # Always enable FSDP-compatible mode
        # This avoids the dynamic checking which could cause issues
        self.fsdp_mode = True
    ...
    def forward(self, batch):
        ...
        # Get node embeddings and ensure they're contiguous for proper reshaping
        x_shape = node_emb.embedding.shape
        # Force contiguous memory layout and clone to avoid potential FSDP issues
        x = node_emb.embedding.contiguous().clone().view(x_shape[0], -1)

        # Energy head processing
        ...
        # FSDP-friendly implementation without torch_scatter
        unique_indices = torch.unique(structure_index)
        energy = torch.zeros(
            len(unique_indices), dtype=e_per_node.dtype, device=e_per_node.device
        )

        # Manually perform the scatter operation
        ...
        # Stress - similar safe implementation
        s_per_node = self.stress_head(x)  # shape [N_atoms, 6]
        stress = torch.zeros(
            len(unique_indices), 6, dtype=s_per_node.dtype, device=s_per_node.device
        )

        # Manual scatter add for stress
        for i, idx in enumerate(unique_indices):
            mask = (structure_index == idx)
